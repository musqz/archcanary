...
- **Detection method:** `--refresh` pages through the `black` and `red` filters and stores the package names in `aur_audit_black.txt`/`aur_audit_red.txt`. Also captures each entry's `pubDateTs` (the flagged version's own AUR publish date — an RSS-style timestamp, not the scan time) into companion `aur_audit_black_dates.txt`/`aur_audit_red_dates.txt` files, so `check_logs` can tell a pacman.log entry that predates the flagged version from one that doesn't, instead of applying a live, current-only verdict to any historical install regardless of age. Yellow is intentionally not synced — too noisy for an automated check without incident history to justify it.
...
- **URL:** https://thehackernews.com/2018/07/arch-linux-aur-malware.html, https://lwn.net/Articles/759461/, https://www.bleepingcomputer.com/news/security/malware-found-in-arch-linux-aur-package-repository/, https://www.securityweek.com/arch-linux-aur-repository-compromised/
- **Date:** Adopted/compromised 2018-06-07; discovered and reverted shortly after, account suspended
- **Author:** Independent public reporting (The Hacker News, LWN.net, BleepingComputer, SecurityWeek) — no single primary Arch mailing-list thread located for this one, but all four outlets independently corroborate the same facts.
- **Content:** A user ("xeactor") adopted the orphaned `acroread` (PDF viewer) package and modified its PKGBUILD to download and run a persistent payload via `curl`, re-checking in every 360 seconds and interfering with systemd configuration. The payload also gathered system reconnaissance data (machine ID, CPU info, pacman state, `uname -a`, `systemctl list-units`). AUR maintainers reverted the change quickly, suspended the account, and found two more packages the same account had recently adopted and modified the same way (names not captured by any source checked).
- **Relevant for:** One of the earliest and most-cited AUR supply-chain incidents — establishes the *orphan takeover* pattern (a dormant/orphaned package gets adopted by a new maintainer who then ships a payload) that later incidents (e.g. the July 2026 wave in section 11) repeat.
- **Not added to any detection list:** `acroread` is a legitimate, actively-maintained package today — the compromise was a bounded window in 2018, not a lasting characteristic of the package. archcanary's list-matching only supports a one-sided cutoff ("ignore anything before this date"), not a bounded start+end compromise window, so adding `acroread` today would flag every legitimate install from 2018 onward indefinitely. Revisit if/when a windowed-cutoff feature exists; until then this is a narrative-only entry.

---

## 13. Google Chrome RAT (2025) — `google-chrome-stable`

- **URL:** https://lwn.net/Articles/1032193/, https://linuxiac.com/arch-aur-under-fire-once-more-as-malware-resurfaces/, https://www.techprovidence.com/aur-fake-chrome-package-rat/
- **Date:** Uploaded and removed within hours, reported 2025-07-31/08-01 (LWN and Linuxiac give dates a day apart — likely a UTC/report-vs-upload distinction, not a discrepancy in the underlying event)
- **Author:** Independent public reporting (LWN.net, Linuxiac, Tech Providence) — no primary aur-general/aur-requests mailing-list thread located despite several targeted searches (see below).
- **Content:** A brand-new AUR account (`forsenontop`, hours old, no other contributions) uploaded `google-chrome-stable`, whose `.install` scriptlet ran a Python command to download and execute remote code every time Chrome launched, silently, with no visible sign to the user. Caught within hours after a handful of upvotes suggest at least some installs happened before removal. Explicitly reported as a **separate** incident from the CHAOS RAT wave (section 6) despite the topical similarity (both RAT-bundled-with-a-legitimate-app impersonation) — LWN frames it as "a similar attack" to "less than a month ago," not a continuation.
- **Relevant for:** A second, distinct browser-impersonation RAT wave; the `.install`-scriptlet-specific delivery mechanism (Python download-and-execute triggered at app launch, not at build time) is a different technique than the PKGBUILD-`build()`-time payloads section 1's campaign used.
- **Added to `lists/community_reports.txt` (2026-08-04), annotated `[community report]` on hit:** `google-chrome-stable`, `chrome`, `google-chrome-bin` (all three consistently named across LWN, Linuxiac, and Tech Providence) and `ttf-mac-fonts-all` (named only in Linuxiac's list, alongside `ttf-ms-fonts-all` — which actually belongs to the separate, earlier CHAOS RAT wave already tracked in section 6/`chaos_rat_packages.txt` — so this attribution is weaker; included anyway since a mis-attributed entry in a best-effort community list costs little and a real fake-sounding package name like this is very unlikely to collide with a legitimate one).
- **Mailing-list search note:** checked `lists.archlinux.org` directly for a primary source — surfaced only unrelated threads (a much larger, separate July 30 2026 batch reported by the aur-audit.wtako.net operator, already covered indirectly via the aur-audit feed in section 9; an unrelated package-deletion request; a Manjaro forum thread that turned out to be an unrelated server-outage false alarm). No primary Arch-run source found for this specific incident; sourced entirely from independent secondary reporting, same as section 11.5 already does for a different incident.

---

## 14. xsnow / xsnow-bin (2026) — Tor-Fetch, Self-Propagating `.install` Payload

- **URL:** https://aur.archlinux.org/cgit/aur.git/tree/.xsnow.install?h=xsnow, https://aur.archlinux.org/cgit/aur.git/tree/.xsnow-bin.install?h=xsnow-bin, https://github.com/manticore-projects/aurscan (Andreas Reichel's AURSCAN, upstream author, released v0.8 the same day specifically to catch this pattern)
- **Date:** Reported 2026-08-23 on the aur-general mailing list; reverted to last-known-good and the account suspended the same day (per Claudia, an AUR admin, on-thread). Specific archive URL not on hand — sourced from a forwarded copy of the thread, not a direct `lists.archlinux.org` lookup.
- **Author (thread):** Saren (initial report — identified the Tor download without needing to analyze the binary), goeb (flagged the self-propagation behavior and suggested auditing for other dot-prefixed `.install` files AUR-wide), Andreas Reichel (root-caused the exact mechanism and shipped an AURSCAN fix), Claudia (AUR admin, reverted + suspended the account).
- **Content:** Both packages' `PKGBUILD` were clean — the payload lived entirely in a dot-prefixed `.<pkgname>.install` scriptlet (`.xsnow.install`/`.xsnow-bin.install`), referenced only via an `install=` line in the PKGBUILD, so a PKGBUILD-only scan (as Reichel's own AURSCAN did before v0.8) missed it. `post_install()`/`post_upgrade()` backgrounded (`</dev/null >/dev/null 2>&1 &`) two routines: (1) install `tor` via `pacman -S --needed --noconfirm`, enable it, then `curl -x socks5h://127.0.0.1:9050` an `.onion` address to fetch a binary (`systemmanager`) straight into `/usr/local/bin/`, `chmod +x` it, and wire it into a new `/etc/systemd/system/SystemManager.service` unit for persistence; (2) iterate every `/home/*/.ssh` and `/root/.ssh` on the machine, use each as an AUR SSH identity (`ssh://aur@aur.archlinux.org`), `list-repos`, and for each one clone it, bump `pkgrel`, copy itself in as that package's own `install=` scriptlet, and `git push` — self-propagating to every other AUR repo any local user's key could reach.
- **Relevant for:** Directly prompted archcanary's `check_pkgbuild_caches` gaining four new patterns (Tor/SOCKS-proxied fetches, download-to-system-path, AUR git SSH remote self-propagation reference, non-interactive `pacman` calls from a scriptlet — PR #220). Confirmed separately that archcanary's existing `.install` file glob (`find -name "*.install"`) already matches dot-prefixed filenames, so the specific "hidden by a leading dot" blind spot that bit Reichel's pre-v0.8 AURSCAN does not apply here — only the payload-pattern coverage was the actual gap.
- **Not added to any detection list:** both packages were reverted to their last-known-good version and the compromised account suspended the same day — they're legitimate, currently-clean packages again, same situation as `acroread` (section 12). archcanary's list-matching has no bounded-window cutoff (only a one-sided "ignore before this date"), so listing them would flag every legitimate install indefinitely. Narrative-only entry, as with `acroread`.

...
- Fix: check [2] (`check_logs`) tagged a still-installed package as `LOG_HIST` — "no longer installed... potential past exposure" — whenever it dropped out of check [1]'s `pacman -Qmq` (foreign-only) lookup, which also happens when a formerly-AUR package graduates into a repo, not just when it's actually removed. `check_current` now also runs one extra batched `pacman -Qq` lookup (`CURRENTLY_INSTALLED_ANY_MAP`, no per-package subprocess) so `check_logs` can tell the two apart: a package that's still installed but no longer foreign gets a new `LOG_MIGRATED` tag with corrected wording instead of the "removed" framing. Deliberately keeps `LOG_HIST`'s exact severity (still bumps `EXIT_CODE`/`_log_ret` to 1) rather than clearing it — "no longer foreign" only means *some* configured repo now owns the name, official or third-party, which is not proof the match is safe; a caught-in-review draft of this fix downgraded it to a zero-severity "not an active risk" INFO note instead, which would have silently cleared a genuine CHAOS-RAT/aur-audit-black match the moment it stopped looking foreign. Unlike `LOG_HIST`, `LOG_MIGRATED` isn't deduped via the `log_hist_seen.txt`/`LOG_HIST_SEEN` mechanism — it's an ongoing current-install fact each scan (like `LOG_HIT`), not a one-time historical event, and sharing that dedup keyspace risked cross-contaminating the two tags on reinstall/removal. Reported on the EndeavourOS forum: `asusctl`/`rog-control-center` moved from the AUR into `extra`, and archcanary kept citing their old AUR install history as a past exposure.
...
- Fix: `archcanary-tui`'s two `sudo` call sites resolved the `archcanary` binary via `command -v` (falling back to `/usr/local/bin/archcanary`) before elevating. `sudo`'s `secure_path` deliberately excludes `~/.local/bin`, but resolving the sudo target through the invoking user's own `$PATH` first is exactly the PATH-hijack `secure_path` exists to prevent — a writable directory ahead of the real binary, or a sourced dotfile, would run as root. Now resolves against a fixed list of known install locations (`~/.local/bin`, `~/bin`, `/usr/local/bin`, `/usr/bin`) instead, never consulting `$PATH`. Contributed by Sanjaya Danushka (NeoArch), caught in review.
- Fix: `--audit-rules-get` returned nothing on an `install.sh --system` install — `configs/audit-rules.conf` was only ever shipped to `/usr/lib/archcanary/` by the AUR `PKGBUILD`, not by `install.sh`'s own root-install path. Now seeded there by both routes.
- Fix: `install.sh --system` left a stale user-scope man page and bash-completion specs behind after a system install, permanently shadowing the system copies (`man` and bash's completion loader both prefer the user-level copy regardless of which is newer — the same shadowing class `--doctor` already warns about). Now cleaned up alongside the existing stale-binary cleanup.
- Fix: `contrib/archcanary-merge-lists.sh` resolved its default npm list and the `archcanary.sh` it shells out to relative to its own directory (`contrib/`) instead of the repo root, and used a `mktemp` template suffix that isn't portable across `mktemp` implementations. Both fixed.
- Added: implemented the documented `--log-file=auto` value — previously `auto` was taken literally, creating and writing to a directory named `auto` instead of the intended default log path. `--help`'s default paths were also corrected for several flags still describing repo-relative defaults (`./package_list.txt`) that haven't been accurate since the config dir moved to `~/.config/archcanary/`, plus a string-indentation bug in its multi-line output.
- Changed: the desktop notifier (`archcanary-notify.service`) now also fires on `RESULT: REVIEW NEEDED`, not just `RESULT: INFECTED` — matching the script's own exit-2 contract, which review-needed results already use.
- Removed: a stray empty file (`octal`) accidentally committed to the repo root.
...
- Added: `--allowlist-{list,add,remove}=package:NAME` — a 5th system-wide allowlist (`/etc/archcanary/package_allowlist.conf`, seeded by both `install.sh --system` and the AUR `PKGBUILD`), applied in checks [1]/[2] (currently-installed packages and pacman.log history). `package_list.txt` and the other threat lists are pure name-match lists with no way to tell a genuine hit apart from a name collision the user has manually verified clean (AUR git history + locally cached PKGBUILD both show no trace of the campaign's malicious commit) — until now the only fix was editing the fetched list file directly, which the next `--refresh` silently reverted. Prompted by a live false-positive: `chipmunk` (the Chipmunk2D physics library) matched `package_list.txt`'s June 2026 AUR-incident list by name only; its actual cached PKGBUILD was the clean, unrelated upstream build. Both checks' WARNING output now hints at `--allowlist-add=package:NAME` inline, matching the existing DKMS/systemd/bpftool/autostart allowlists. Also added to `archcanary-tui`'s "Manage allowlists" menu alongside the other four.
- Fix: the new package allowlist above shipped in the initial PR without two of its three install paths — the AUR `PKGBUILD`'s `package()` never seeded `/etc/archcanary/package_allowlist.conf` (nor listed it in `backup=()`), and `archcanary-tui`'s allowlist menu still only offered the original four. Caught before release, not after — the exact same class of gap as v0.1.31's `archcanary-tui` packaging miss.
...
- Fix: the base64/rev-tr/printf pipe-to-shell patterns (2, 4, 7) and their yay-hook ports read `| sha256sum` / `| sha512sum` as `| sh` — so the standard Chrome extension-id line `echo $key | base64 -d | sha256sum | head -c32 | tr 0-9a-f a-p` (dozens of `-bin` packages) tripped a WARNING, and a hard *abort* in the yay hook. The shell name is now word-anchored: matched directly after a `|`/`|&`, or through one privilege/env wrapper (`| sudo bash`, `| env -i sh`), but not as another command's argument (`| grep bash`, `| sudo pacman -S bash`) or across a `;`/`&&`/`||` boundary. `base64 -di` bundled flags are caught, the decode output must be piped onward (not redirected to a file), `curl`/`wget` must sit at a command position (a `$curl_result` / `libcurl` substring no longer aborts), and Pattern 2 skips fully-commented lines. Indirection through `xargs` or a detached wrapper operand stays out of scope — a tripwire for the common `curl … | sh` shape, not a sandbox.
- Fix: the yay hook's `%f[…]` frontier-pattern anchors (printf-`| sh`, `sudo -u root`, pipe-to-shell) were dead code — yay 13 runs the hook under gopher-lua v1.1.2, which has no `%f` and parses it as literal `f`, so they never matched in production (they pass under the system Lua the test suite uses). Reworked to bracket character classes plus a trailing sentinel, and a lint test now rejects any `%f[` in `configs/yay-init.lua`.
...
- Fix: `check_pkgbuild_caches` Pattern 9 (duplicate `source=()` declaration) flagged a legitimate `source=(...)` followed by `source+=(...)` array append as a duplicate, since the regex collapsed both operators to the same key — reported live on `vscodium-bin`, whose PKGBUILD does exactly this to add one extra file. Now only a bare `=` triggers the check (it's the only operator that actually discards prior content); a `+=` staged before a later bare `=` still correctly flags, since that's the same `storageexplorer-bin` attack shape (v0.1.28) with the operators swapped.
...
- Added: `--scan-user=NAME` — runs `--scan-all-homes`'s same npm/bun/yarn/pnpm/pkgbuild/autostart checks against one specific user's home instead of enumerating everyone. Repeatable (`--scan-user=alice --scan-user=bob`), de-duped, needs root, mutually exclusive with `--scan-all-homes`. For checking a single newly-created or flagged account without sweeping every local user. Its final output is a full "Check summary: USER x" table per named user (instead of `--scan-all-homes`'s single table folded across everyone), so a multi-name run shows exactly which check flagged which user — `--scan-all-homes` itself is unaffected, still the shared table.
- Changed: `configs/yay-init.lua`'s `AURPostDownload` hook now blocks on a `read` "press Enter to continue" prompt after any non-aborted verdict (clean or a red warning), turning it into a checkpoint you have to acknowledge instead of a line that scrolls off under makepkg/cargo build spam — reported live where a clean verdict got lost under hundreds of `Compiling` lines during a big Rust build. Skipped when stdin isn't a terminal, so scripted/piped installs pass through unaffected. Marker bumped to `(v9)`.

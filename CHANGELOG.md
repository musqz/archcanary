...
- Added: `check_pkgbuild_caches` gained three rules from Andreas Reichel's static-rules scan of all ~111,000 live AUR packages (2026-08-25) — nothing malicious found, but each shape sidesteps a real safety property. (1) `sudo`/`doas`/`pkexec` invoked from a PKGBUILD or `.install` (`tarah` runs `sudo cp … /usr/bin` past fakeroot); (2) a `source=` URL on a forge merge-request/pull-request diff endpoint, whose content can change after review (`freetype2-wps`) — a hash-pinned `/commit/<sha>.patch` is not matched; (3) a `source=` entry that no integrity array checksums, whose URL carries neither the literal `pkgver` nor a `$pkgver` reference (`power-menu-bin` declares `0.1.2`, fetches `v0.1.1` with `md5sums=SKIP`). Sources and checksums are paired positionally the way aurscan's CHK-005 does it, so a real hash on a `.desktop` no longer masks a `SKIP` on the wrong-version tarball; an entry is "verified" if any array (`md5sums`, `sha256sums`, …) holds a real hash for it, and VCS entries, an entry whose own `.sig`/`.asc` is in the array, remote `.patch`/`.diff` (that's rule 2's job), and a group whose arrays don't pair one-to-one are all left alone. Rules 1–2 are also in the yay hook (`configs/yay-init.lua`, marker `(v10)`), both as warnings.

...
- Added: `--scan-user=NAME` — runs `--scan-all-homes`'s same npm/bun/yarn/pnpm/pkgbuild/autostart checks against one specific user's home instead of enumerating everyone. Repeatable (`--scan-user=alice --scan-user=bob`), de-duped, needs root, mutually exclusive with `--scan-all-homes`. For checking a single newly-created or flagged account without sweeping every local user. Its final output also gets a "Per-user summary" table (alongside the existing per-check one) so a multi-name run shows at a glance whose result is whose — `--scan-all-homes` is unaffected, still just the per-check table.
